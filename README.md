...
```powershell
$d="$HOME\NetworkAudioBridge"; New-Item -Force -ItemType Directory $d|Out-Null; @("nab.exe","nab-recv.exe","start.bat","start-recv.bat") | ForEach-Object { irm "https://github.com/kennichicode/network-audio-bridge/releases/latest/download/$_" -OutFile "$d\$_" }; $ws=New-Object -ComObject WScript.Shell; $s=$ws.CreateShortcut("$HOME\Desktop\NAB 送受信.lnk"); $s.TargetPath="$d\start.bat"; $s.WorkingDirectory=$d; $s.Save(); $s2=$ws.CreateShortcut("$HOME\Desktop\NAB 受信機v2.lnk"); $s2.TargetPath="$d\start-recv.bat"; $s2.WorkingDirectory=$d; $s2.Save(); Write-Host "完了！デスクトップに2つのショートカットを作りました"
```

実行後、デスクトップに以下の2つのショートカットが作られます：
- **「NAB 送受信」** — `nab.exe`（送信・受信・双方向）
- **「NAB 受信機v2」** — `nab-recv.exe`（受信専用・ドリフト補正付き）
...
for f in nab nab-recv start.command start-recv.command; do \
  curl -L "https://github.com/kennichicode/network-audio-bridge/releases/latest/download/$f" -o ~/NetworkAudioBridge/$f; \
done && \
chmod +x ~/NetworkAudioBridge/nab ~/NetworkAudioBridge/nab-recv ~/NetworkAudioBridge/start.command ~/NetworkAudioBridge/start-recv.command && \
cp ~/NetworkAudioBridge/start.command ~/Desktop/ && \
cp ~/NetworkAudioBridge/start-recv.command ~/Desktop/ && \
echo "完了！デスクトップに2つのファイルを置きました"
```

実行後、デスクトップに以下が置かれます：
- **「start.command」** — `nab`（送信・受信・双方向）
- **「start-recv.command」** — `nab-recv`（受信専用・ドリフト補正付き）
