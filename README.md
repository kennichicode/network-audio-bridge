...
Windows には **送信機 `nab.exe` のみ** をインストールします。

```powershell
$d="$HOME\NetworkAudioBridge"; New-Item -Force -ItemType Directory $d|Out-Null; @("nab.exe","start.bat") | ForEach-Object { curl.exe -sSL "https://github.com/kennichicode/network-audio-bridge/releases/latest/download/$_" -o "$d\$_" }; $ws=New-Object -ComObject WScript.Shell; $s=$ws.CreateShortcut("$HOME\Desktop\NAB 送受信.lnk"); $s.TargetPath="$d\start.bat"; $s.WorkingDirectory=$d; $s.Save(); Write-Host "完了！デスクトップに「NAB 送受信」ショートカットを作りました"
```

実行後、デスクトップに **「NAB 送受信」** ショートカットが作られます（`nab.exe` = 送信・受信・双方向すべてに対応）。
